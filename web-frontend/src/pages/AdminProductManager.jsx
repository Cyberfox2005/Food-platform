...
export default function AdminProductManager() {
  ...
  const save = event => {
    ...
    else setItems(previous => [...previous, { ...form, id: Date.now(), price: Number(form.price), image: initialProducts[0].image, dietary: [], rating: 0, reviewsCount: 0, availability: true }]);
    setForm(empty);
    setEditingId(null);
  };
  const beginEdit = item => { setEditingId(item.id); setForm({ name: item.name, description: item.description, price: item.price, category: item.category }); };
